...
public class RandomWalk {
	...
	/** Used to store the particle's start location. */
	...
	RandomWalk(int gridSize) {
		...
		stuck = false;
		startPoint = new Point(this.gridSize -1,this.gridSize - 1);
	...
	/**
	 * The step method randomly chooses a valid next step, restarting at Start Point if no valid next step exists, and continues until the particle reaches it's goal.
	 */
	public void step() {
	...
	public ArrayList<Point> getPath() {
		ArrayList<Point> pathClone = new ArrayList<Point>(Arrays.asList(new Point[this.path.size()]));
		Collections.copy(pathClone, this.path);
		return pathClone;
	}
	...
	public String toString() {
		...
		//displayPath += "\n\nIt took " + attempts + " attempts for the particle to reach it's goal!";
		return displayPath;
	}
}
